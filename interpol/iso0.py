...
def grad(inp, g, bound: List[Bound], extrapolate: int = 1):
    ...
    return torch.zeros([batch, channel] + oshape + [dim], dtype=inp.dtype, device=inp.device)


@torch.jit.script
def pushgrad(inp, g, shape: Optional[List[int]], bound: List[Bound], extrapolate: int = 1):
    """
    ...
    return torch.zeros([batch, channel] + shape, dtype=inp.dtype, device=inp.device)
...
def hess(inp, g, bound: List[Bound], extrapolate: int = 1):
    ...
    return torch.zeros([batch, channel] + oshape + [dim, dim], dtype=inp.dtype, device=inp.device)
