...
from interpol import add_identity_grid_, grid_count, grid_grad, grid_pull, grid_push
...
def test_gradcheck_grad(device, dim, bound, interpolation):
    ...
    assert gradcheck(grid_grad, (vol, grid, interpolation, bound, extrapolate), **kwargs)
...
def test_gradcheck_pull(device, dim, bound, interpolation):
    ...
    vol.requires_grad = True
    grid.requires_grad = True
    assert gradcheck(grid_pull, (vol, grid, interpolation, bound, extrapolate), **kwargs)
...
def test_gradcheck_push(device, dim, bound, interpolation):
    ...
    vol.requires_grad = True
    grid.requires_grad = True
    assert gradcheck(grid_push, (vol, grid, shape, interpolation, bound, extrapolate), **kwargs)
...
def test_gradcheck_count(device, dim, bound, interpolation):
    ...
    grid.requires_grad = True
    assert gradcheck(grid_count, (grid, shape, interpolation, bound, extrapolate), **kwargs)
