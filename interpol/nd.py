...
from .jit_utils import cartesian_prod, list_prod_int, make_sign, sub2ind_list
...
@torch.jit.script
def inbounds_mask(extrapolate: int, grid, shape: List[int]) -> Optional[Tensor]:
    ...
    if extrapolate in (0, 2):  # no / hist
        ...
        if extrapolate == 2:
            threshold = 0.5 + tiny
        mask = torch.ones(grid.shape[:-1], dtype=torch.bool, device=grid.device)
...
@torch.jit.script
def get_weights(
    grid, bound: List[Bound], spline: List[Spline], shape: List[int], grad: bool = False, hess: bool = False
) -> Tuple[
    List[List[Tensor]],
...
@torch.jit.script
def pull(inp, grid, bound: List[Bound], spline: List[Spline], extrapolate: int = 1):
    """
    ...
    # initialize
    out = torch.zeros([batch, channel, grid.shape[1]], dtype=inp.dtype, device=inp.device)

    # iterate across nodes/corners
    range_nodes = [torch.as_tensor([d for d in range(n)]) for n in [s.order + 1 for s in spline]]
    if dim == 1:
...
@torch.jit.script
def push(inp, grid, shape: Optional[List[int]], bound: List[Bound], spline: List[Spline], extrapolate: int = 1):
    """
    inp: (B, C, *ishape) tensor
    g: (B, *ishape, D) tensor
    ...
    # initialize
    out = torch.zeros([batch, channel, list_prod_int(shape)], dtype=inp.dtype, device=inp.device)
...
@torch.jit.script
def grad(inp, grid, bound: List[Bound], spline: List[Spline], extrapolate: int = 1):
    """
    inp: (B, C, *ishape) tensor
    ...
    # initialize
    out = torch.zeros([batch, channel, grid.shape[1], dim], dtype=inp.dtype, device=inp.device)
...
@torch.jit.script
def pushgrad(inp, grid, shape: Optional[List[int]], bound: List[Bound], spline: List[Spline], extrapolate: int = 1):
    """
...
@torch.jit.script
def hess(inp, grid, bound: List[Bound], spline: List[Spline], extrapolate: int = 1):
    """
    inp: (B, C, *ishape) tensor
    grid: (B, *oshape, D) tensor
    bound: List{D}[Bound] tensor
    ...
    # precompute weights along each dimension
    weights, grads, hesss, coords, signs = get_weights(grid, bound, spline, shape, grad=True, hess=True)

    # initialize
    out = torch.zeros([batch, channel, grid.shape[1], dim, dim], dtype=inp.dtype, device=inp.device)
    ...
    for nodes in all_nodes:
        ...
        for d in range(dim):
            ...
            # apply weights
            for dd, (weight, hess1, n) in enumerate(zip(weights, hesss, nodes)):
                if d == dd:
                    hess11 = hess1[n]
