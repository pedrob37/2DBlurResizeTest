...
from .autograd import GridCount, GridGrad, GridPull, GridPush, SplineCoeff, SplineCoeffND
from .jit_utils import meshgrid, movedim1
...
def grid_pull(input, grid, interpolation="linear", bound="zero", extrapolate=False, prefilter=False):
    ...
    if backend.jitfields and jitfields.available:
        return jitfields.grid_pull(input, grid, interpolation, bound, extrapolate, prefilter)
    ...
    if not input.dtype.is_floating_point:
        ...
        for label in input.unique():
            ...
            if prefilter:
                input = spline_coeff_nd(soft, interpolation=interpolation, bound=bound, dim=dim, inplace=True)
            soft = GridPull.apply(soft, grid, interpolation, bound, extrapolate)
            out[soft > pmax] = label
            pmax = torch.max(pmax, soft)
    else:
        if prefilter:
            input = spline_coeff_nd(input, interpolation=interpolation, bound=bound, dim=dim)
        out = GridPull.apply(input, grid, interpolation, bound, extrapolate)

    return _postproc(out, shape_info, mode="pull")


def grid_push(input, grid, shape=None, interpolation="linear", bound="zero", extrapolate=False, prefilter=False):
    ...
    """
    if backend.jitfields and jitfields.available:
        return jitfields.grid_push(input, grid, shape, interpolation, bound, extrapolate, prefilter)
    ...
    if prefilter:
        out = spline_coeff_nd(out, interpolation=interpolation, bound=bound, dim=dim, inplace=True)
    return _postproc(out, shape_info, mode="push")


def grid_count(grid, shape=None, interpolation="linear", bound="zero", extrapolate=False):
...
def grid_grad(input, grid, interpolation="linear", bound="zero", extrapolate=False, prefilter=False):
    ...
    """
    if backend.jitfields and jitfields.available:
        return jitfields.grid_grad(input, grid, interpolation, bound, extrapolate, prefilter)
...
def spline_coeff(input, interpolation="linear", bound="dct2", dim=-1, inplace=False):
    ...
    # as we know.
    if backend.jitfields and jitfields.available:
        return jitfields.spline_coeff(input, interpolation, bound, dim, inplace)

    out = SplineCoeff.apply(input, bound, interpolation, dim, inplace)
    return out


def spline_coeff_nd(input, interpolation="linear", bound="dct2", dim=None, inplace=False):
    """Compute the interpolating spline coefficients, for a given spline order
    ...
    # as we know.
    if backend.jitfields and jitfields.available:
        return jitfields.spline_coeff_nd(input, interpolation, bound, dim, inplace)

    out = SplineCoeffND.apply(input, bound, interpolation, dim, inplace)
    return out


grid_pull.__doc__ = grid_pull.__doc__.format(interpolation=_doc_interpolation, bound=_doc_bound)
grid_push.__doc__ = grid_push.__doc__.format(interpolation=_doc_interpolation, bound=_doc_bound)
grid_count.__doc__ = grid_count.__doc__.format(interpolation=_doc_interpolation, bound=_doc_bound)
grid_grad.__doc__ = grid_grad.__doc__.format(interpolation=_doc_interpolation, bound=_doc_bound)
...
def identity_grid(shape, dtype=None, device=None):
    ...
    """
    mesh1d = [torch.arange(float(s), dtype=dtype, device=device) for s in shape]
...
def add_identity_grid_(disp):
    ...
    mesh1d = [torch.arange(s, dtype=disp.dtype, device=disp.device) for s in spatial]
...
def affine_grid(mat, shape):
    ...
    if nb_dim != len(shape):
        raise ValueError(
            "Dimension of the affine matrix ({}) and shape ({}) " "are not the same.".format(nb_dim, len(shape))
        )
    if mat.shape[-2] not in (nb_dim, nb_dim + 1):
        raise ValueError(
            "First argument should be matrces of shape "
            "(..., {0}, {1}) or (..., {1}, {1}) but got {2}.".format(nb_dim, nb_dim + 1, mat.shape)
        )
