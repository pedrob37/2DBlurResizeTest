...
from .jit_utils import inbounds_mask_1d, inbounds_mask_2d, inbounds_mask_3d, make_sign, sub2ind_list
...
@torch.jit.script
def push3d(inp, g, shape: Optional[List[int]], bound: List[Bound], extrapolate: int = 1):
    """
    inp: (B, C, iX, iY, iZ) tensor
...
def grad3d(inp, g, bound: List[Bound], extrapolate: int = 1):
    ...
    # gather
    inp = inp.reshape(list(inp.shape[:2]) + [-1])
    out = torch.empty([batch, channel] + list(g.shape[-2:]), dtype=inp.dtype, device=inp.device)
...
@torch.jit.script
def pushgrad3d(inp, g, shape: Optional[List[int]], bound: List[Bound], extrapolate: int = 1):
    """
...
def hess3d(inp, g, bound: List[Bound], extrapolate: int = 1):
    ...
    # gather
    inp = inp.reshape(list(inp.shape[:2]) + [-1])
    out = torch.empty([batch, channel, g.shape[-2], dim, dim], dtype=inp.dtype, device=inp.device)
    outx, outy, outz = out.unbind(-1)
...
@torch.jit.script
def push2d(inp, g, shape: Optional[List[int]], bound: List[Bound], extrapolate: int = 1):
    """
    inp: (B, C, iX, iY) tensor
    g: (B, iX, iY, 2) tensor
...
@torch.jit.script
def pushgrad2d(inp, g, shape: Optional[List[int]], bound: List[Bound], extrapolate: int = 1):
    """
...
@torch.jit.script
def push1d(inp, g, shape: Optional[List[int]], bound: List[Bound], extrapolate: int = 1):
    """
    inp: (B, C, iX, iY) tensor
    g: (B, iX, iY, 2) tensor
    shape: List{2}[int], optional
...
@torch.jit.script
def pushgrad1d(inp, g, shape: Optional[List[int]], bound: List[Bound], extrapolate: int = 1):
    """
    inp: (B, C, iX, 1) tensor
    g: (B, iX, 1) tensor
...
def hess1d(inp, g, bound: List[Bound], extrapolate: int = 1):
    ...
    """
    batch = max(inp.shape[0], g.shape[0])
    return torch.zeros([batch, inp.shape[1], g.shape[1], 1, 1], dtype=inp.dtype, device=inp.device)
