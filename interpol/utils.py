...
def expanded_shape(*shapes, side="left"):
    ...
    *shapes : sequence[int]
    ...
    def error(s0, s1):
        raise ValueError("Incompatible shapes for broadcasting: {} and {}.".format(s0, s1))
    ...
    for i, shape1 in enumerate(shapes):
        ...
        if side == "left":
            ...
            shape1 = [*shape1, *ones]
        shape = [max(s0, s1) if s0 == 1 or s1 == 1 or s0 == s1 else error(s0, s1) for s0, s1 in zip(shape, shape1)]
