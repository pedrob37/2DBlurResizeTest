...
def dct2_initial(inp, pole: float, dim: int = -1, keepdim: bool = False):
    ...
    poles = torch.as_tensor(pole, dtype=inp.dtype, device=inp.device)
    poles = poles.pow(torch.arange(1, n - 1, dtype=inp.dtype, device=inp.device)) + poles.pow(
...
@torch.jit.script
def filter(inp, bound: CoeffBound, poles: List[float], dim: int = -1, inplace: bool = False):
    if not inplace:
        inp = inp.clone()
...
@torch.jit.script
def spline_coeff(inp, bound: int, order: int, dim: int = -1, inplace: bool = False):
...
@torch.jit.script
def spline_coeff_nd(inp, bound: List[int], order: List[int], dim: Optional[int] = None, inplace: bool = False):
    """Compute the interpolating spline coefficients, for a given spline order
